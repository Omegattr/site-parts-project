...
car.addEventListener('load', (event) => {
  ...
  colorButtons.forEach((button) => {
    button.addEventListener('click', (e) => {
      ...
      /* 車体を回転させるアニメーションを追加 */
      const currentOrbit = car.getAttribute('camera-orbit');
      // 現在の角度を取得し、数値部分だけ抜き出す
      let [deg, ...rest] = currentOrbit.split(' ');
      let baseDeg = parseFloat(deg);
      // 360度加算して一周回す
      let newDeg = baseDeg + 360;
      // アニメーション用にgsapで属性を変化させる
      gsap.to(car, {
        duration: 1.2,
        ease: Power2.easeInOut,
        attr: { 'camera-orbit': `${newDeg}deg ${rest.join(' ')}` },
        onComplete: () => {
          // 1周回った後、元の角度に戻す（360度分引く）
          car.setAttribute('camera-orbit', `${baseDeg}deg ${rest.join(' ')}`);
        }
      });
